...
- Seventeen tests pass locally. The previous 16-test checkpoint passed on both Orins. A new test covers frozen thermal/container admission.
...
- Next: commit/push this fix; sync both Orins, verify tests/clean state/no containers, and launch a new detached campaign at `runs/campaign-v1-r2` using `--config config/campaign-v1.json --blocks 12 --repeats 2 --warmups 2 --seed 20260908` and all three committed traces. Record driver PID/log paths here immediately. Never update a checkout while its campaign is running.
